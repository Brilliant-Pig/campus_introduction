<template>
    <div>
      <div class="center-box">
        ...
        <div class="question-container">
          ...
          <el-input v-model="input" class="question-input" placeholder="" />
        </div>
  ...
  const fetchQuestions = async () => {
    try {
      const response = await axios.get('http://your-api-endpoint/questions')
      questions.value = response.data
      drawer.value = true
    } catch (error) {
      console.error('获取问题列表失败:', error)
      ElMessage.error('获取问题列表失败') // 使用Element Plus的消息提示
    }
  }
